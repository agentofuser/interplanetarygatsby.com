    ...
    async path => {
      ...
      contents = contents
        ...
        .replace(
          /(["'])\/__GATSBY_IPFS_PATH_PREFIX__\/([^'"]*?)(['"])/g,
          (matches, g1, g2, g3) =>
            ` __GATSBY_IPFS_PATH_PREFIX__ + ${g1}/${g2}${g3}`
        )
...
const injectScriptInHtmlFiles = async () => {
  ...
  const scriptBuffer = await readFileAsync(
    path.resolve(__dirname, 'runtime/head-script.js')
  )
...
exports.onPreBootstrap = ({ store, reporter }) => {
  ...
  if (!/\/?__GATSBY_IPFS_PATH_PREFIX__/.test(config.pathPrefix)) {
    reporter.panic(
      'The pathPrefix must be set to __GATSBY_IPFS_PATH_PREFIX__ in your gatsby-config.js file'
    )
  }
